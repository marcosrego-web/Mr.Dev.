...
document.addEventListener('DOMContentLoaded', function() {
	...
	if (navigator.userAgent.match(/(iPod|iPhone|iPad)/)) {
		const mrwidPages = document.querySelectorAll('.mr-theme .mr-layout.mr-windowheight .mr-pages');
		for (id = 0; id < mrwidPages.length; id++) {
			mrwidPages[id].classList.add('ios-scrolling');
		}
		const mrwidContainers = document.querySelectorAll('.mr-theme .mr-layout.mr-expandactive .mr-pages:not(.mr-perliner) .mr-item.active .mr-container');
