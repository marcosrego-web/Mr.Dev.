										...
										if($excludeinclude == 0 AND !in_array($itemid, $itemselect) OR $excludeinclude == 1 AND in_array($itemid, $itemselect) ) {
											...
											*/
											if($itemimage == 0) { //No image
												...
												if($getimg) {
													...
													/*$showimage = '><figure class="mrwid-image" '.((in_array('background',$imagestype))?'style="background-image: url('.$getimg.');':'src="'.$getimg).((in_array('thumbnail',$imagestype))?$addimagemaxwidth.$addimagemaxheight:'').'"'.$imgalt.'>'.$imgcaption.'</figure';*/
												}
											}
											/*
											Title starts here
											*/
											...
											if($itemstitle == 2) { //Category title
												...
												$showitemtitle = '<'.$titletag.' class="mrwid-title">'.'<a href="'.$itemLink.'">'.$itemtitle.((in_array("artcount", $itemoptions) && is_numeric($num_articles))?' <small>('. $num_articles .')</small>':"").'</a>'.'</'.$titletag.'>';
											}
											/*
											...
											*/
											...
											echo  '<li class="itemid-'.$itemid.' '.$itemslug.' '.$mrclasses.' mr-wid '.$mrcurrent.'" '.((in_array("url", $itemoptions))?'url='.$itemLink:"").'><div class="mrwid-container"'.$showimage.'>'.$showitemtitle.'<div class="mrwid-content">'.$showitemdesc.$bottomlinktext.'</div></div></li>';
