										...
										if($excludeinclude == 0 AND !in_array($itemid, $itemselect) OR $excludeinclude == 1 AND in_array($itemid, $itemselect) ) {
											...
											*/
											if($itemimage == 0) { //No image
												...
												if($getimg) {
													...
													/*$showimage = '><figure class="mr-image" '.((in_array('background',$imagestype))?'style="background-image: url('.$getimg.');':'src="'.$getimg).((in_array('thumbnail',$imagestype))?$addimagemaxwidth.$addimagemaxheight:'').'"'.$imgalt.'>'.$imgcaption.'</figure';*/
												}
											}
											/*
											Title starts here
											*/
											...
											if($itemstitle == 2) { //Category title
												...
												$showitemtitle = '<'.$titletag.' class="mr-title">'.'<a href="'.$itemLink.'">'.$itemtitle.((in_array("artcount", $itemoptions) && is_numeric($num_articles))?' <small>('. $num_articles .')</small>':"").'</a>'.'</'.$titletag.'>';
											}
											/*
											...
											*/
											if($itemlink == 1) { //No bottom link
												...
												$bottomlinktext = '<div class="mr-link"><a class="'.$bottomlinkclasses.'" href="'.$itemLink.'" title="'. $cattitle .'">'.$bottomlink.'</a></div>';
											}
											/*
