				...
				$currentLink = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
					...
					*/
					if($maintitle == 3) { //Theme and layout title
						$content .= '<'.$titletag.'>'.$theme . ' - ' . $layout.'</'.$titletag.'>';
					} else if($maintitle == 4) { //Theme title
						$content .= '<'.$titletag.'>'.$theme.'</'.$titletag.'>';
					} else if($maintitle == 5) { //Layout title
						$content .= '<'.$titletag.'>'.$layout.'</'.$titletag.'>';
					} else {
					}
					/*
				...
				if ( ! empty( $itemlist ) ) {
						...
						*/
						...
						if($tabs == 2) { //Parent Items Tabs
							...
							foreach ( $itemlist as $key => $tab) {
								$tabid = $tab->id;
								$content .='<li class="parentitemid-'.$tabid.' '.$tab->alias.' mr-tab">'.$tab->title.'</li>';
							}
							$content .='</ul>';
						}
						$itemcount = 0;
										...
										*/
										if($excludeinclude == 0 AND !in_array($itemid, $itemselect) OR $excludeinclude == 1 AND in_array($itemid, $itemselect) ) {
											...
											*/
											...
											$content .= '<li class="itemid-'.$itemid.' '.$itemslug.' '.$mrclasses.' mr-item '.$mrcurrent.'" '.((in_array("url", $itemoptions))?'url='.$itemLink:"").'><div class="mr-container"'.$showimage.'>'.$showitemtitle.'<div class="mr-content">'.$showitemdesc.$bottomlinktext.'</div></div></li>';
